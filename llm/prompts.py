...
def build_graphrag_messages(title: str, body: str, tags: str, retrieved: list,
    ...
    2. Instruksi eksplisit DUAL-CONSTRAINT (kalau `require_grounding=True`,
       default -- perilaku asli sejak awal Kondisi C):
    ...
    blok instruksi teks, TIDAK mengubah bagaimana `retrieved` diformat:
      - True : dua constraint di atas (grounding + citation), perilaku asli.
      - False: HANYA instruksi citation (identik build_rag_messages() Kondisi
        B, require_citation=True) -- TANPA instruksi grounding "jangan
    ...
    if retrieved:
        ...
        if require_grounding:
            ...
            # Sama seperti build_rag_messages() (Kondisi B, require_citation=True)
    ...
    system_content = ("You are an expert in software engineering with much experience on "
                       "programming.")
    if require_grounding:
...
def build_lightrag_messages(title: str, body: str, tags: str, retrieved: list,
    ...
    adaptasi LightRAG) -- tiap item konteks diberi label "[SO-{question_id}]"
    (sama seperti Kondisi C), TAPI metadata transparansi yang ditampilkan
    adalah `source_stage` ("low_level"/"high_level"), BUKAN trust_weight/hop
    (Kondisi D tidak punya trust score).

    `require_grounding` (default True) HANYA mengganti blok instruksi teks,
    TIDAK mengubah cara `retrieved` diproses/diformat:
      - True: instruksi DUAL-CONSTRAINT (grounding + citation) disalin APA
        ADANYA dari build_graphrag_messages() Kondisi C.
      - False: HANYA instruksi citation, disalin APA ADANYA dari
        build_rag_messages() Kondisi B (require_citation=True) -- TANPA
        instruksi grounding.
    """
    tag_list = _clean_tags(tags)
    ...
    if retrieved:
        context_lines = []
        for r in retrieved:
            meta = f" ({r['source_stage']})" if r.get("source_stage") else ""
            context_lines.append(f"[SO-{r['question_id']}]{meta} {r['chunk_text']}")
        context_block = "\n\n".join(context_lines)

        if require_grounding:
            # Disalin apa adanya dari build_graphrag_messages() (Kondisi C).
            context_section = (
                f"Here is context retrieved from the Stack Overflow community knowledge "
                f"graph that may help you answer, each labeled with its source thread id "
                f"(e.g. [SO-1234]):\n\n{context_block}\n\n"
            ...
            # Disalin apa adanya dari build_rag_messages() (Kondisi B,
